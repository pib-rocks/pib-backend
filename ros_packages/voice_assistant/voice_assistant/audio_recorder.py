...
# ros_packages/voice_assistant/voice_assistant/audio_loop.py
...
class GeminiAudioLoop:
    """Continuously routes mic ↔ speaker via Gemini Live API."""
    ...
    async def listen_audio(self):
        """Read mic input and enqueue to out_queue."""
    ...
    async def send_realtime(self):
        """Stream queued mic chunks into the Gemini Live session."""
        while True:
            chunk = await self.out_queue.get()
            await self.session.send_realtime_input(audio=chunk)

    async def receive_audio(self):
        """Read API audio chunks, route to in_queue for playback."""
        while True:
            async for turn in self.session.receive():
                for resp in turn:
                    if resp.audio:
                        self.in_queue.put_nowait(resp.audio.data)
                    # if you want to handle resp.text here, you can

            # flush if interrupted
            while not self.in_queue.empty():
                self.in_queue.get_nowait()

    async def play_audio(self):
        """Pull from in_queue and play to speaker."""
    ...
    async def run(self):
        """Orchestrate the Live API session and all I/O tasks."""
        api_key=""
        client = genai.Client(api_key=api_key)  # ensure API key / ADC is set
        try:
