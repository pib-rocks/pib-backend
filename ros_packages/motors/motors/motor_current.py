...
class Motor_current(Node):
    ...
    def __init__(self):
        ...
        self.current_publisher_ = self.create_publisher(
            DiagnosticStatus, "motor_current", 10
        )
        self.publish_ = self.create_timer(
            1.0 / self.frequency_, self.callback_publisher
        )
        ...
        try:
            ...
            self.ipcon.connect(cfg.TINKERFORGE_HOST, cfg.TINKERFORGE_PORT)
    ...
    def callback_publisher(self):
        self.iterate_servos(
            self.servo1, self.current_values_servo1_, self.motors_servo1_, 0
        )
        self.iterate_servos(
            self.servo2, self.current_values_servo2_, self.motors_servo2_, 1
        )
        self.iterate_servos(
            self.servo3, self.current_values_servo3_, self.motors_servo3_, 2
        )
