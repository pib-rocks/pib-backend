...
from datatypes.msg import (
    ProxyRunProgramFeedback,
    ProxyRunProgramResult,
    ProxyRunProgramStatus,
)
...
class ProxyProgramNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.feedback_publisher = self.create_publisher(
            ProxyRunProgramFeedback, "proxy_run_program_feedback", feedback_profile
        )
        self.result_publisher = self.create_publisher(
            ProxyRunProgramResult, "proxy_run_program_result", 10
        )
        self.status_publisher = self.create_publisher(
            ProxyRunProgramStatus, "proxy_run_program_status", 10
        )

        # one service for making a request to the action (start), and one for cancelling a goal (stop)
        self.start_program = self.create_service(
            ProxyRunProgramStart, "proxy_run_program_start", self.start_program_callback
        )
        self.stop_program = self.create_service(
            ProxyRunProgramStop, "proxy_run_program_stop", self.stop_program_callback
        )
        ...
        self.run_program_client = ActionClient(
            self, RunProgram, "run_program", feedback_sub_qos_profile=feedback_profile
        )
    ...
    def start_program_callback(
        ...
        def forward_feedback_to_publisher(feedback_message) -> None:
            ...
            proxy_feedback = ProxyRunProgramFeedback(
                proxy_goal_id=proxy_goal_id, output_lines=feedback.output_lines
            )
        ...
        def receive_result(result_future: Future) -> None:
            ...
            self.result_publisher.publish(
                ProxyRunProgramResult(
                    exit_code=result.exit_code, proxy_goal_id=proxy_goal_id
                )
            )
        ...
        for proxy_goal_id, tuple in self.id_to_goal.items():
            ...
            if status != next_status:
                self.status_publisher.publish(
                    ProxyRunProgramStatus(
                        proxy_goal_id=proxy_goal_id, status=next_status
                    )
                )
