...
from schema.pose_schema import poses_schema, pose_schema, pose_schema_motor_positions_only, create_pose_schema, pose_schema_name_only, pose_schema_without_motor_positions
...
def create_pose():
    ...
    db.session.commit()
    try: return pose_schema.dump(pose), 201
...
def get_all_poses():
    ...
    try: return jsonify({"poses": poses_schema.dump(poses)})
    except Exception: abort(500)
...
def get_motor_positions_of_pose(pose_id: str):
    ...
    try: return pose_schema_motor_positions_only.dump(pose)
    except Exception: abort(500)
...
def rename_pose(pose_id: str):
    ...
    db.session.commit()
    try: return pose_schema_without_motor_positions.dump(pose)
    except Exception: abort(500)