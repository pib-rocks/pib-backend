    ...
    try:
        with sqlite3.connect(database) as conn:
            ...
            for uid, bricklet_number in zip(new_uids, bricklet_numbers):
                if not uid:
                    continue
                cur.execute("UPDATE bricklet SET uid = ? WHERE (bricklet_number) = ?", (f"uid{bricklet_number}", bricklet_number),)

            for uid, bricklet_number in zip(new_uids, bricklet_numbers):
                if not uid:
                    continue
                cur.execute("UPDATE bricklet SET uid = ? WHERE (bricklet_number) = ?", (uid, bricklet_number),)

            rows = cur.execute("SELECT uid, bricklet_number FROM bricklet").fetchall()
            print(rows)
