"""Verify seed_db includes hermes-agent and Gemini 3.6 Flash assistant models."""
...
def test_seed_includes_gemini_3_6_flash_assistant_model(app_ctx):
    from model.assistant_model import AssistantModel

    gemini = AssistantModel.query.filter_by(api_name="gemini-3.6-flash").one()
    assert gemini.visual_name == "Gemini 3.6 Flash"
    assert gemini.has_image_support is False


def test_get_all_assistant_models_returns_gemini_3_6_flash(app_ctx):
    ...
    assert "gemini-3.6-flash" in api_names
