#!/bin/bash
# Delegate PR-1809 to cursor-agent and print what actually changed.
set -u
cd /home/pib/opencode/pib-backend-PR-1809
export CURSOR_API_KEY="${CURSOR_API_KEY:-$(grep -E '^CURSOR_API_KEY=' ~/.hermes/.env | cut -d= -f2- | tr -d '"' | tr -d "'")}"
echo "=== PR-1809 Delegation $(date -Is) ==="
...
timeout 3300 /home/pib/.local/bin/cursor-agent --trust --model auto \
    ...
    -p "$(cat .hermes/plans/PR-1809-delegation-prompt.txt)" 2>&1 | tail -120
echo "CURSOR_EXIT=$?"
echo "--- Status ---"
git status --short
echo "--- Commits ---"
